  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <article className="card p-5">
          <SectionTitle title="References benchmark" isDemo={Boolean(latestBenchmark?.is_demo)} />
          {latestBenchmark?.benchmark_json.references?.length ? (
            <div className="mt-4 grid gap-3">
              {latestBenchmark.benchmark_json.references.map((reference) => (
                <div className="rounded-md border border-[#ded8cc] bg-[#fffefa] p-4" key={reference.title}>
                  <h3 className="font-semibold">{reference.title}</h3>
                  <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-[#6b7280]">{reference.region}</p>
                  <p className="mt-2 text-sm leading-6 text-[#5f675f]">{reference.why_relevant}</p>
                </div>
              ))}
            </div>
          ) : (
            <EmptyState text="Genere un benchmark pour nourrir la direction artistique du projet." />
          )}
        </article>

        <article className="card p-5">
          <SectionTitle title="Plan texture et prompt realiste" isDemo={Boolean(latestPlan?.is_demo)} />
          {latestPlan ? (
            <div className="mt-4 space-y-4">
              <p className="rounded-md border border-[#ded8cc] bg-[#fffefa] p-4 text-sm leading-6 text-[#39423a]">
                {latestPlan.realistic_plan_prompt}
              </p>
              <DetailList title="Zones" items={latestPlan.plan_json.zones} />
              <DetailList title="Materiaux" items={latestPlan.plan_json.materials} />
              <DetailList title="Vegetal" items={latestPlan.plan_json.planting} />
              <DetailList title="Validation" items={latestPlan.plan_json.validation_notes} />
            </div>
          ) : (
            <EmptyState text="Genere un plan texture pour preparer le rendu ou la validation finale." />
          )}
        </article>
      </section>
...
function SectionTitle({ title, isDemo }: { title: string; isDemo?: boolean }) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      {isDemo ? (
        <span className="rounded-full border border-[#d8a555] bg-[#fff7df] px-3 py-1 text-xs font-bold uppercase tracking-wide text-[#8a5a00]">
          Mode démo IA
        </span>
      ) : null}
    </div>
